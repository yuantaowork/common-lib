<?php
namespace Cmexpro\Common\Lib\MsgQ;

use Cmexpro\Common\Lib\Common\ScopeExit;
use Cmexpro\Common\Lib\Prometheus\Application\TaskHistogram;
...
class MsgQHelper
{
     ...
     * @return AMQPChannel
     */
    protected static function getChannel($conf_key = 'local')
    {
     ...
     * @param string $conf_key
     */
    public static function publishStr_old($exchange_name, $publish_routing_key, $body, $conf_key = 'local' /* 指定连接的MQ-server */)
    {
     ...
     */
    public static function publishStr($exchange_name, $publish_routing_key, $body, $conf_key = 'local' /* 指定连接的MQ-server */)
    {
    ...
    public static function publishStr_nothrow($exchange_name, $publish_routing_key, $body, $conf_key = 'local' /* 指定连接的MQ-server */)
    {
        try {
     ...
     */
    public static function publishStrWithConfirm($exchange_name, $publish_routing_key, $body, $conf_key = 'local' /* 指定连接的MQ-server */)
    {
    ...
    public static function publishStrWithConfirm_nothrow($exchange_name, $publish_routing_key, $body, $conf_key = 'local' /* 指定连接的MQ-server */)
    {
        $ret = 'exception';
        try {
    ...
    public static function subscribeStr($procFunc /* 业务逻辑处理函数: void f($body) */,
                                        ...
                                        $conf_key = 'local', $delayed_routing_ms = 0)
    {
        // declare(ticks = 100); //经测试, 新版php-amqplib不再需要定义ticks
        pcntl_async_signals(true); //PHP7.1后支持这种更高效的信号处理设置

        // 注册SIGINT,SIGTERM,SIGHUP的处理函数
        pcntl_signal(SIGINT, 'Cmexpro\Common\Lib\MsgQ\MsgQHelper__consumer_sigterm_func');
        pcntl_signal(SIGTERM, 'Cmexpro\Common\Lib\MsgQ\MsgQHelper__consumer_sigterm_func');
