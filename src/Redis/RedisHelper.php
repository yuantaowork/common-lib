...
use Cmexpro\Common\Lib\Config\IConfigCache;
use Cmexpro\Common\Lib\Log\LOG;
use Cmexpro\Common\Lib\Log\BackTrace;
...
class RedisHelper
{
    ...
    public static function getConn_nothrow($conf_key, $pingCheck = false)
    {
        try {
            ...
            return new RedisHelper(); // 返回一个空壳对象
        }
    }
    ...
    protected function _connect()
    {
        try {
            ...
            $timeout = $this->current_conf['timeout'] ?? 3.0; // 默认超时3秒
            $persistent = $this->current_conf['persistent'] ?? false;
            $persistent_id = $this->current_conf['persistent_id'] ?? null;

            $this->redis = new \Redis();

            $this->redis->{($persistent ? 'pconnect' : 'connect')}($host, $port, $timeout, $persistent_id);
            if (! empty($auth)) {
