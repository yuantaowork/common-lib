...
class LOG
{
    ...
    public static function initLogFilePrefix($prefix = null)
    {
        self::$logFilePrefix = $prefix;
    }

    /**
     * 得到一个logger, 底层文件名是 "${file_base_name}${$logFilePrefix}__$name.log"
     * @param string $name
     * @return Logger
     */
    private static function initAndGetLogger($name)
    {
        if (empty(self::$loggers[$name])) {
            ...
            if ($host_prefix) {
                $file_prefix = !empty(self::$logFilePrefix) ? self::$logFilePrefix :
                    (!empty($_SERVER['HTTP_HOST']) ? $_SERVER['HTTP_HOST'] : '');
                if (!empty($file_prefix)) {
                    if ($file_prefix[-1] == '/') {
                        $file_base_name .= $file_prefix; // 如果prefix带目录, 直接拼接
                    } else {
                        $file_base_name .= ($file_prefix . '__'); // 否则, 多拼接两个下划线
                    }
                } // else do nothing
            }
    ...
    private static $priorTraceId;

    public static function resetPriorTraceId($reqTraceId = null)
    {
        self::$priorTraceId = $reqTraceId;
    }

    public static function getTraceId()
    {
        // request scope的traceId优先级更高
        if (!empty(self::$priorTraceId)) {
            return self::$priorTraceId;
        }
    ...
    private static $priorLoggerName;

    public static function resetPriorLoggerName($reqLoggerName = null)
    {
        self::$priorLoggerName = $reqLoggerName;
    }

    public static function __callStatic($name, $arguments)
    {
        // request scope的loggerName优先级更高
        $logger = self::initAndGetLogger(!empty(self::$priorLoggerName) ? self::$priorLoggerName : $name);
        ...
        $message = sprintf('T<%s> %s - %s - %d', self::getTraceId(), $msg, $trace, self::$pid);
